...
class UuidUtil {
  ...
  static List<int> mathRNG({int seed}) {
    int rand;
    List<int> b = new List<int>(16);

    Random _rand = (seed == null) ? new Random() : new Random(seed);
    for (var i = 0; i < 16; i++) {
      if ((i & 0x03) == 0) {
  ...
  static List cryptoRNG() {
    int nBytes = 32;
    List<int> pwBytes = new List<int>(nBytes);

    List<int> bytes = mathRNG();
